...
- There is an Add Another Item button that opens Add Item for the same room.
...
- PDF Generation: Uses expo-print. Landscape US Letter with a 2×2 inventory grid; items with multiple photos also get full-page photo sheets. Local photos are embedded as base64 JPEG data URIs.
- CSV Generation: The app converts the list of item data arrays into a comma-separated text string (.csv) entirely within TypeScript utility logic (includes photo count and Drive image URLs when present).
